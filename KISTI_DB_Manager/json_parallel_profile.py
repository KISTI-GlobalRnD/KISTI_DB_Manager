    ...
    *,
    ...
    row = {
        ...
        "timings_ms": {
            ...
            "rust_arrow.table_assemble": _as_int(timings.get("rust_arrow.table_assemble"), 0),
            ...
            "rust_arrow.id_compaction": _as_int(timings.get("rust_arrow.id_compaction"), 0),
            "rust_arrow.table_write": _as_int(timings.get("rust_arrow.table_write"), 0),
...
def _aggregate_worker_attempts(
    *,
    ...
    parser_backends = sorted({str(item.get("rust_parser_backend") or "") for item in attempt_rows if item.get("rust_parser_backend")})
    parser_effective = sorted(
        {
            str(item.get("rust_parser_backend_effective") or "")
            for item in attempt_rows
...
def _render_parallel_profile_markdown(summary: Mapping[str, Any]) -> str:
    ...
    lines.append(f"- rust_parser_backend: `{summary.get('rust_parser_backend')}`")
    ...
    lines.append(
        ...
        "rust_arrow.py_to_json_ms | json.flatten_ms | rust_arrow.table_assemble_ms | rust_arrow.columnar_merge_ms | "
        "json.parquet.persist_ms | rust_arrow.id_compaction_ms | rust_arrow.table_write_ms | rust_arrow.arrow_build_ms | rust_arrow.parquet_write_ms | "
    ...
    for row in summary.get("runs") or []:
        ...
        lines.append(
            ...
            f"{_as_int(timings.get('rust_arrow.table_assemble'), 0)} | "
            f"{_as_int(timings.get('rust_arrow.columnar_merge'), 0)} | "
            f"{_as_int(timings.get('json.parquet.persist'), 0)} | "
            f"{_as_int(timings.get('rust_arrow.id_compaction'), 0)} | "
            f"{_as_int(timings.get('rust_arrow.table_write'), 0)} | "
...
def profile_parallel(
    *,
    ...
    rust_parser_backend: str | None = None,
    profile_top: int = 8,
    repeat: int = 1,
    ...
    from .rust_arrow_backend import normalize_parser_backend, parse_backend_list
    ...
    for order, (flatten_backend, worker, repeat_index) in enumerate(plan, start=1):
        ...
        parser_requested = normalize_parser_backend(
            rust_parser_backend if rust_parser_backend is not None else data_config.get("rust_parser_backend", "serde-json")
        )
        data_config["rust_parser_backend"] = parser_requested if str(flatten_backend) == "rust-arrow" else "serde-json"
    ...
    summary: dict[str, Any] = {
        ...
        "rust_parser_backend": normalize_parser_backend(rust_parser_backend) if rust_parser_backend is not None else None,
